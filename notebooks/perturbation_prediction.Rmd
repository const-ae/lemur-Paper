...
```{r, paged.print=FALSE}
# double_cross_pl <- inter_pred_dat %>%
#   mutate(pred_better = case_when(
#     abs(abs(value - ground_truth) - abs(ground_truth - additive_model)) < threshold ~ "~ equal",
#     abs(value - ground_truth) - abs(ground_truth - additive_model) > threshold ~ "add better",
#     abs(value - ground_truth) - abs(ground_truth - additive_model) < -threshold ~ "pred better",
...
#     annotate(geom = "text", x = -0.98, y = 0.7, label = "add. better", hjust = 0, vjust = 1, size = font_size_tiny / .pt) +
#     annotate(geom = "text", x = 0.98, y = 0.7, label = "pred.\nbetter", hjust = 1, vjust = 1, size = font_size_tiny / .pt) +
#     annotate(geom = "text", x = 0.98, y = -0.7, label = "add. better", hjust = 1, vjust = 0, size = font_size_tiny / .pt) +
#     annotate(geom = "text", x = -0.98, y = -0.7, label = "pred.\nbetter", hjust = 0, vjust = 0, size = font_size_tiny / .pt) +
#     ggrastr::rasterize(geom_point(aes(color = interaction_type), size = 0.2, stroke = 0), dpi = 600) +
#     annotate(geom = "path", x = c(-Inf, 0, 1-threshold*4), y = c(threshold, threshold, 1)) +
#     annotate(geom = "path", x = -c(-Inf, 0, 1-threshold*4), y = -c(threshold, threshold, 1)) +
#     annotate(geom = "path", x = c(Inf, threshold, 1-threshold*3), y = c(threshold, threshold, 1)) +
#     annotate(geom = "path", x = -c(Inf, threshold, 1-threshold*3), y = -c(threshold, threshold, 1)) +
#     facet_wrap(vars(method)) +
#     coord_fixed(xlim = c(-1, 1), ylim = c(-1, 1)) +
#     guides(color = guide_legend(override.aes = list(size = 2))) +
#     labs(x = "Interaction type (observed minus additive)", y = "Predicted interaction type\n(prediction minus additive)",
#          color = "")
# 
# double_cross_pl
```
...
inter_pred_dat %>%
  ...
  filter(pred_better != "~ equal") %>%
  dplyr::count(pred_better, prediction_label, interaction_type, prediction_type, method) %>%
  mutate(interaction_type = factor(interaction_type, levels = c("suppressive", "additive", "synergistic")),
         prediction_type = factor(prediction_type, levels = c("suppressive", "additive", "synergistic"))) %>%
  mutate(n = scales::label_comma()(n)) %>%
  ggplot(aes(x = interaction_type, y = prediction_type)) +
    geom_tile(aes(fill = prediction_label), show.legend = FALSE) +
    geom_text(aes(label = n), size = font_size / .pt) +
    scale_y_discrete(drop = FALSE) +
    facet_wrap(vars(method, pred_better), drop = FALSE)
```
...
plot_assemble(
  ...
  add_plot(pert_pred_comparison, x = 0, y = 57, width = 90, height = 50),
  ...
  width = 170, height = 107, units = "mm", show_grid_lines = FALSE,
