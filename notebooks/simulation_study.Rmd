...
pl1 <- tibble(celltype = celltype_assign, condition = cond_assign, 
  ...
  ggplot() +
    ...
    annotate_labels_along(origin = fit$linear_coefficients[,1], dir = ctrl_space, labels = "$R(\\textrm{control})$", at = 2.5,
    ...
    annotate_labels_along(origin = fit$linear_coefficients[,2], dir = trt_space, labels = "$R(\\textrm{treated})$", at = 2.5,
                          hjust = 0, offset = 0.15, size = font_size_small/.pt, color = "#898989") +
    ...
    annotate("text", x = 1.2, y = 1.7, label = "$\\operatorname{Exp}^{(\\textrm{Gr})}_{o}(x B)$", hjust = 0, size = font_size_small/.pt) +
    annotate("text", x = 0.3, y = 0.4, label = "$x \\Gamma$", hjust = 0.5, size = font_size_small/.pt) +
...
pl2 <- tibble(celltype = celltype_assign, condition = cond_assign,emb = fit$embedding[1,]) %>%
  ...
  ggplot(aes(x = emb, y = diff)) +
    geom_segment(aes(xend = emb, yend = 0, color = condition), lwd = 0.1, show.legend = FALSE) +
...
plot_assemble(
  ...
  # add_graphic("../illustrations/cell_type_kidneys.pdf", x = 10, y = 5, width = 53, height = 32, units = "mm"),
