...
abs_max <- max(abs(quantile(de_plot_data$de, c(0.9, 0.1))))
...
jun_de_pl <- de_plot_data %>%
  ...
  ggplot(aes(x = umap[,2], y = umap[,1])) +
    ...
    facet_grid(vars(inside), labeller = labeller(inside = as_labeller(c("in" = "Inside Neighb.", "out" = "Outside Neighb."))),
               switch="y") +
    small_axis("UMAP", fontsize = font_size_small) +
    theme(legend.position = "bottom", legend.margin = margin(t = -2, unit = "mm")) +
    guides(color = guide_colorbar(barheight = unit(1, "mm"), barwidth = unit(10, "mm"), title.vjust = 1)) +
    labs(color = "$\\Delta$")
