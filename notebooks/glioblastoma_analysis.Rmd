...
cell_type_dot_plot <- as_tibble(as.matrix(t(expr_mat))) %>%
  ...
  ggplot() +
    ...
    guides(color = guide_colorbar(barheight = unit(1, "mm"), barwidth = unit(5, "mm"), title.vjust = 0.25, label.position = "top", label.vjust = -2),
           size = guide_bins(theme = theme(legend.text = element_text(vjust = 3)))) +
...
plot_assemble(add_text("(A) Glioblastoma cell type assignment", x = 1.5, y = 1.5, vjust = 1, fontsize = font_size, fontface = "bold"),
              add_plot(my_get_legend(cell_type_legend_suppl), x = 3, y = 5, height = 5),
...
source_dat <- as_tibble(colData(fit_small)) %>%
    mutate(umap = reducedDim(fit_small, "UMAP")) %>%
    unpack_2d()
...
```
```{r}
comparison_dat <- as_tibble(colData(psce)) %>%
...
plot_assemble(add_text("(A) Expression pattern of selected genes under panobinostat treatment", x = 1.5, y = 1.5, vjust = 1, fontsize = font_size, fontface = "bold"),
              ...
              add_text("(B) Predicted differential expression pattern ($\\Delta = \\hat{\\matr{Y}}^{\\textrm{pan}} - \\hat{\\matr{Y}}^{\\textrm{ctrl}}$)",
                       x = 1.5, y = 57, vjust = 1, fontsize = font_size, fontface = "bold"),
...
de_plot_data2 <- as_tibble(colData(fit_small), rownames = "cell_id") %>%
  ...
  tidylog::filter(cell_type == "Tumor cells") %>%
  sample_frac(size = 1) 
...
de_plots2 <- de_plot_data2 %>%
  ...
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    ...
    scale_x_continuous(limits = quantile(de_plot_data2$umap[,1], c(0.01, 1)) * 1.1) +
    scale_y_continuous(limits = c(-6, 6.8)) +
    small_axis(label = "", fontsize = font_size_tiny, arrow_length = 3) +
...
diff_pl <- diff_df %>% 
  ggplot(aes(x = origin, xend = origin, y = 0, yend = panobinostat - ctrl)) +
    ...
    ggsignif::geom_signif(comparison = list(c("inside", "outside")), 
                          annotations = paste0("$p=7\\times10^{-5}$"),
                          textsize = font_size_tiny / .pt, y_position = 0.01, vjust = -0.1) +
    scale_y_continuous(limits = c(-0.1, 0.075), expand = expansion(add = 0), breaks = c(-0.1, 0)) +
    scale_x_discrete(labels = c("inside" = "Nbh.", "outside" = "other")) +
    labs(subtitle = "$ \\text{Diff. in diff.}$\n($\\Delta_{\\text{nbh}} - \\Delta_{\\text{other}}$)",
         y = "$\\Delta$") +
...
```{r, paged.print=FALSE}
nei_long <- lemur.utils::neighborhoods_to_long_data(nei_pan_tumor_small[nei_pan_tumor_small$name == "ENSG00000135363",], fit_small,
                                        cell_names = colnames(qs::qread("../benchmark/output/glioblastoma/fit_hvg10k-small.qs")))
lemur.utils::fit_pivot_longer(fit_small, genes = "ENSG00000135363", colnames = "cell_id") %>%
  tidylog::left_join(nei_long, by = c("gene_id" = "name", "cell_id" = "cell")) %>%
  dplyr::count(patient_id, inside, is_tumor = cell_type == "Tumor cells") %>%
  arrange(is_tumor, inside, patient_id)
```
...
volcano_enrichment_dat <- as_tibble(glm_de_res) %>%
...
volcano_enrichment_pl <- volcano_enrichment_dat %>%
...
```{r, paged.print=FALSE}
writexl::write_xlsx(list("Panel B" = source_dat, 
                         "Panel C (UMAP)" = unpack_2d(de_plot_data2),
                         "Panel C (pseudobulk)" = comparison_data2,
                         "Panel D" = volcano_enrichment_dat),
                    path = "../source_data/glioblastoma_results.xlsx")
```
...
plot_assemble(
  ...
  add_plot(de_plots2 + guides(color = "none"), x = 0, y = 112.5, width = 61, height = 40),
  ...
  add_plot(comparison_pl2, x = 61, y = 114, width = 45, height = 41.6),
