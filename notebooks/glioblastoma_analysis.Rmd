...
condition_colors <- structure(c("#66c2a5", "#fc8d62", "#8da0cb"), names = c("ctrl", "panobinostat", "etoposide"))
...
cell_type_dot_plot <- as_tibble(as.matrix(t(expr_mat))) %>%
  ...
  ggplot() +
    ...
    theme(axis.title = element_blank(), legend.position = c(0,-0.3), legend.direction = "horizontal",
...
chr_ratio_pl <- colData(fit_small) %>%
  ...
  ggplot(aes(x = ratio)) +
    ...
    facet_wrap(vars(origin), nrow = 1, labeller = as_labeller(c(chr_10_ratio = "Chr.~10 Ratio", chr_7_ratio = "Chr.~7 Ratio"))) +
...
mc_biplot_marker <- as_tibble(colData(fit_small)) %>%
  ...
  ggplot(aes(x = emb[,1], y = emb[,2])) +
    ...
    geom_segment(data = proj_df %>% filter(gene != "HIST3H2A"), aes(x = 0, y = 0, xend = V1 * 200, yend = V2 * 200, color = condition), linewidth = 1.2, show.legend = FALSE) +
    geom_point(data = proj_df %>% filter(gene != "HIST3H2A") %>% arrange(desc(condition)), aes(x = V1 * 200, y = V2 * 200, color = condition), size = 0.6, show.legend = FALSE) +
    scale_color_manual(values = condition_colors) +
    geom_text(data = manual_text_pos %>% filter(gene != "HIST3H2A"), aes(x = V1, y = V2, hjust = hjust, vjust = vjust, label = gene), size = font_size_small / .pt) +
    # coord_fixed(ylim = c(-20, 25), xlim = c(-40, 20), clip = "off") +
    small_axis(label = "$Z$ space", xlim = c(-40, 40), ylim = c(-20, 25)) +
    labs(x = "$Z_1$", y ="$Z_2$")
...
mc_biplot_diff <- as_tibble(colData(fit_small)) %>%
  ...
  ggplot(aes(x = emb[,1], y = emb[,2])) +
    ...
    scale_color_manual(values = cell_type_colors) +
    ggnewscale::new_scale_colour() +
    geom_segment(data = proj_df %>% filter(gene == "HIST3H2A") %>% arrange(desc(condition)), aes(x = 0, y = 0, xend = V1 * 2000, yend = V2 * 2000, color = condition), linewidth = 1.2, show.legend = FALSE) +
    geom_point(data = proj_df %>% filter(gene == "HIST3H2A") %>% arrange(desc(condition)), aes(x = V1 * 2000, y = V2 * 2000,color = condition), size = 0.6, show.legend = FALSE) +
    scale_color_manual(values = condition_colors) +
    geom_text(data = manual_text_pos %>% filter(gene == "HIST3H2A"), aes(x = V1, y = V2, hjust = hjust, vjust = vjust, label = gene), size = font_size_small / .pt) +
    # coord_fixed(ylim = c(-20, 25), xlim = c(-40, 20), clip = "off") +
    small_axis(label = "$Z$ space", xlim = c(-40, 40), ylim = c(-25, 20)) +
    labs(x = "$Z_1$", y ="$Z_2$")
...
umap_orig_pc_pl <- make_dim_pl("UMAP", color_by = condition, axis_label = "UMAP") +
  geom_text(data = . %>% filter(cell_type == "Tumor cells") %>% summarize(umap = matrix(colMedians(umap), nrow = 1), .by = patient_id),
...
de_plots <- de_plot_data %>%
  ...
  group_by(gene) %>%
  group_map(\(data, key){
    abs_max <- max(abs(quantile(data$de, c(0.9, 0.1))))
    data$gene <- key[[1]][1]
    ggplot(data, aes(x = umap[,1], y = umap[,2])) +
      ggrastr::rasterise(geom_point(aes(color = de), size = 0.05, stroke = 0), dpi = 600) +
      scale_colour_gradient2_rev(limits = c(-1, 1) * signif(abs_max, 1), oob = scales::squish, breaks = c(-1, 0, 1) * signif(abs_max, 1)) +
      facet_grid(vars(inside), vars(gene), labeller = labeller(inside =as_labeller( c("in" = "Inside Neighb.", "out" = "Outside Neighb"))), switch = "y")  +
...
de_plot <- de_plot_data %>%
  filter(gene == "HIST3H2A") %>%
  mutate(de = scale_sym(de, ignore_extreme = 0.1)) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
...
expr_plots <- expr_plot_data %>%
  ...
  group_map(\(data, key){
    ...
    ggplot(data, aes(x = umap[,1], y = umap[,2])) +
      ...
      facet_grid(vars(condition), vars(gene), switch = "y")  +
      scale_x_continuous(limits = range(umap_fit[,1]) * 1.1) +
      scale_y_continuous(limits = range(umap_fit[,2]) * 1.1) +
      small_axis(label = "", fontsize = font_size_tiny, xlim = range(umap_fit[,1]), ylim = range(umap_fit[,2]), arrow_length = 3) +
      guides(color = guide_colorbar(barheight = unit(1, "mm"), title = "")) +
...
comparison_pl <- as_tibble(colData(psce)) %>%
  ...
  ggplot(aes(x = condition_short, y = expr)) +
    ...
    coord_cartesian(clip = "off") +
    # guides(x = guide_axis(angle = 90)) +
    labs(subtitle = "Pseudobulked expr.\n(inside neighborhood)") +
...
condition_legend <- ggplot(enframe(condition_colors) %>% filter(name != "etoposide"), aes(x = 0, y = 0, color = name)) + 
  geom_point() + 
...
plot_assemble(
  ...
  add_plot(cowplot::get_legend(condition_legend), x = 3, y = 31 , height = 34),
  add_text("UMAP of $Y$", x = 25 + row1_umap_width * 0.5, y = 30, vjust = 1, hjust = 0.5, fontsize = 7),
  add_text("UMAP of $Z$ if $S = I$", x = 25 + row1_umap_width * 1.5, y = 30, vjust = 1, hjust = 0.5, fontsize = 7),
  add_text("UMAP of $Z$ if $S$ estimated with harmony", x = 25 + row1_umap_width * 2.5, y = 30, vjust = 1, hjust = 0.5, fontsize = 7),
  ...
  add_plot(cowplot::get_legend(cell_type_legend), x = 3, y = 70, height = 40),
  ...
  add_text("(C) Cell type assignment", x = 1.5, y = 111, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_plot(cell_type_dot_plot + guides() , x = 0, y = 114, width = 47, height = 26),
  add_plot(chr_ratio_pl, x = 7, y = 144, width = 40, height = 13),
  
  add_text("(D) Multi-condition biplot", x = 49, y = 111, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_text("Marker Genes", x = 70, y = 114.3, hjust = 0.5, vjust = 1, fontsize = font_size_small),
  add_plot(mc_biplot_marker, x = 47, y = 114, width = 42, height = 20),
  add_text("Diff. Gene", x = 70, y = 134.3, hjust = 0.5, vjust = 1, fontsize = font_size_small),
  add_plot(mc_biplot_diff, x = 47, y = 134, width = 42, height = 20),
   
  add_text("(E) Predicted differential expression of HIST3H2A for each cell", x = 86.5, y = 111, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_plot(de_plot, x = 85, y = 110, width = 60, height = 40),
  add_graphic("../illustrations/manual_divergent_colorscaleArtboard 1.pdf", x = 100,  y = 145, units = "mm"),
  add_text("down", x = 101, y = 146.9, vjust = 1, fontsize = font_size_small, hjust = 1),
  add_text("regulation", x = 101, y = 148.9, vjust = 1, fontsize = font_size_small, hjust = 1),
  add_text("up", x = 125.5, y = 146.9, vjust = 1, fontsize = font_size_small, hjust = 0),
  add_text("regulation", x = 125.5, y = 148.9, vjust = 1, fontsize = font_size_small, hjust = 0),
  add_plot(comparison_pl, x = 142, y = 113.5, width = 28, height = 37),

  add_text("(F) Panobinostat stimulates LMO2 down-regulation in a subpopulation of tumor cells", x = 1.5, y = 158, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_plot(de_plots2, x = 0, y = 160, width = 61, height = 40),
  add_graphic("../illustrations/manual_divergent_colorscaleArtboard 1.pdf", x = 15,  y = 196, units = "mm"),
  add_text("down", x = 16, y = 197.9, vjust = 1, fontsize = font_size_small, hjust = 1),
  add_text("regulation", x = 16, y = 199.9, vjust = 1, fontsize = font_size_small, hjust = 1),
  add_text("up", x = 40.5, y = 197.9, vjust = 1, fontsize = font_size_small, hjust = 0),
  add_text("regulation", x = 40.5, y = 199.9, vjust = 1, fontsize = font_size_small, hjust = 0),
  add_plot(comparison_pl2, x = 61, y = 161, width = 45, height = 41.8),
  add_plot(diff_pl, x = 104, y = 162, width = 25, height = 40.5),
  
  
  add_text("(G) Characterization of tumor\n\\quad\\quad subpopulation", x = 130, y = 158, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_plot(cowplot::get_legend(volcano_enrichment_pl + guides(color = guide_legend(override.aes = list(size = 1))) + labs(color = "")),
           x = 133, y = 164.5, width = 36, height = 4),
  add_plot(volcano_enrichment_pl + guides(color = "none"), x = 129, y = 168, width = 41, height = 37),
  
  width = 170, height = 205, units = "mm", show_grid_lines = FALSE, latex_support = TRUE,
