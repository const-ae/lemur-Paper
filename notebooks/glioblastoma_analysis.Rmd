...
cell_type_colors <- structure(c("#FAC1BD", "#ffffb3", "#80b1d3", "#b3de69"), names = c("Tumor", "T cell", "Oligodendrocytes", "Myeloid"))
...
cell_type_dot_plot <- as_tibble(as.matrix(t(expr_mat))) %>%
  ...
  ggplot() +
    ...
    theme(axis.title = element_blank(), legend.position = c(-0.38,-0.2), legend.direction = "horizontal",
...
mc_biplot <- as_tibble(colData(fit_small)) %>%
  ...
  ggplot(aes(x = -emb[,1], y = -emb[,2])) +
    ...
    small_axis(label = "$Z$ space", xlim = c(-40, 20), ylim = c(-27, 32)) +
    labs(x = "$Z_1$", y ="$Z_2$")
...
comparison_pl <- as_tibble(colData(psce)) %>%
  ...
  group_map(\(data, key){
    ggplot(data, aes(x = condition_short, y = expr)) +
      ...
      theme(axis.title.x = element_blank(), strip.background = element_blank(), strip.text = element_blank(),
...
plot_assemble(
  ...
  add_text("UMAP of $Z$ before alignment", x = 25 + row1_umap_width * 1.5, y = 30, vjust = 1, hjust = 0.5, fontsize = 7),
  add_text("UMAP of $Z$ after alignment", x = 25 + row1_umap_width * 2.5, y = 30, vjust = 1, hjust = 0.5, fontsize = 7),
  ...
  add_plot(cell_type_dot_plot + guides() , x = 0, y = 114, width = 37, height = 37),
  add_text("(D) Multi-condition biplot", x = 39, y = 111, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_plot(mc_biplot, x = 37, y = 114, width = 42, height = 40),
   
  add_text("(E) Cluster-free differential expression of HIST3H2A", x = 86.5, y = 111, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_text("Panobinostat causes", x = 91, y = 116, vjust = 1, fontsize = 7),
  add_graphic("../illustrations/manual_divergent_colorscaleArtboard 1.pdf", x = 113.5,  y = 113, units = "mm"),
  add_plot(de_plots[[1]], x = 85, y = 116, width = 60, height = 40),
  add_plot(comparison_pl[[1]], x = 145, y = 119, width = 25, height = 36),

  add_text("(F) Panobinostat stimulates HERPUD1 up-regulation in a subpopulation of tumor cells", x = 1.5, y = 158, vjust = 1, fontsize = font_size, fontface = "bold"),
  add_plot(de_plots2, x = 0, y = 162, width = 61, height = 40),
  add_plot(comparison_pl2, x = 61, y = 162, width = 45, height = 40),
  add_plot(diff_pl, x = 104, y = 162, width = 25, height = 40),
  add_text("(G) Subpopulation markers\n\\quad\\quad in control condition", x = 130, y = 158, vjust = 1, fontsize = font_size, fontface = "bold"),
