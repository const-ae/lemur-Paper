...
if(! file.exists("../benchmark/output/glioblastoma/fit_hvg10k-small.qs")){
  ...
  as_tibble(nei_eto_tumor) %>% arrange(pval)
  
  pan_affect_genes <- c("HIST3H2A", "CD84", "MBP", "CCL7")
  tumor_bin_genes <- c("PDIA4")
  ...
  inside_pdia4_nei <- colnames(fit[,fit$colData$cell_type == "Tumor"]) %in% filter(nei_pan_tumor_small, name == "ENSG00000155660")$neighborhood[[1]]
  psce_pdia4 <- glmGamPoi::pseudobulk(fit[,fit$colData$cell_type == "Tumor"], group_by = vars(patient_id, condition, inside = inside_pdia4_nei), n = n())
  ...
  qs::qsave(psce_pdia4, "../benchmark/output/glioblastoma/fit_hvg10k-pdia4_psce.qs")
...
psce_pdia4 <- qs::qread("../benchmark/output/glioblastoma/fit_hvg10k-pdia4_psce.qs")
...
manual_text_pos <- tribble(
  ...
  "HIST3H2A",         "ctrl",    -2,    2,     1,       0,
  "HIST3H2A", "panobinostat",   -14,  -10,   0.2,       1,
)
...
mc_biplot <- as_tibble(colData(fit_small)) %>%
  ...
  ggplot(aes(x = emb[,1], y = emb[,2])) +
    ...
    geom_segment(data = proj_df, aes(x = 0, y = 0, xend = V1, yend = V2, color = condition), linewidth = 1.2, show.legend = FALSE) +
    scale_color_manual(values = condition_colors) +
    geom_text(data = manual_text_pos, aes(x = V1 * 1.2, y = V2 * 1.2, hjust = hjust, vjust = vjust, label = gene), size = font_size_small / .pt) +
...
pan_volcano <- nei_pan_mod %>%
  as_tibble() %>%
  mutate(sel_gene = name %in% nei_pan_small$name) %>%  
  arrange(sel_gene) %>%
  mutate(lfc = ifelse(lfc < -4, -Inf, lfc)) %>%
  ggplot(aes(x = lfc, y = pval)) +
    ggrastr::rasterize(geom_point(aes(color = sel_gene, size = sel_gene), stroke = 0, show.legend = FALSE), dpi = 300) +
    geom_hline(yintercept = max(nei_pan_mod$pval[nei_pan_mod$adj_pval < 0.1])) +
    scale_color_manual(values = c("TRUE" = "red", "FALSE" = "black")) +
    scale_size_manual(values = c("TRUE" = 1.2, "FALSE" = 0.6)) +
    scale_y_continuous(limits = c(1, NA), expand = expansion(mult = c(0, 0.05)), trans = neg_log10_trans,
                       breaks = 10^seq(0, -8, by = -2), labels = \(x) glue("$10^{{{log10(x)}}}$")) +
    labs(x = "Log fold change", y = "p-value")

# nei_pan_mod %>%
#   # mutate(did_adj_pval2 = p.adjust(ifelse(adj_pval < 0.1, did_pval, NA), method = "BH")) %>%
#   as_tibble() %>%
#   ggplot(aes(x = n_cells, y = pval)) +
#     geom_point(aes(color = did_lfc, size = did_pval < 0.01),  stroke = 0) +
#     geom_hline(yintercept = max(nei_pan_mod$pval[nei_pan_mod$adj_pval < 0.1])) +
#     colorspace::scale_color_continuous_diverging(limits = c(-2, 2), oob = scales::oob_squish, palette = "Purple-Green") +
#     scale_size_manual(values = c("TRUE" = 1.2, "FALSE" = 0.6)) +
#     scale_y_continuous(limits = c(1, NA), expand = expansion(mult = c(0, 0.05)), trans = neg_log10_trans,
#                        breaks = 10^seq(0, -8, by = -2), labels = \(x) glue("$10^{{{log10(x)}}}$")) +
  ...
  group_map(\(data, key){
    ggplot(data, aes(x = umap[,1], y = umap[,2])) +
      ...
      geom_density2d(data = de_regions_maps %>% filter(gene == key[[1]]) %>% mutate(inside = "in"),  breaks = 0.2, linewidth = 0.75,
                     ...
                     show.legend = FALSE) +
      scale_color_gradient2(limits = c(-1, 1), oob = scales::squish, breaks = c(-1, 0, 1)) +
...
expr_plots <- expr_plot_data %>%
  ...
  mutate(gene = factor(gene, levels = c(pan_genes_order, "PDIA4"))) %>%
  group_by(gene) %>%
  group_map(\(data, key){
    data %>% mutate(gene = key[[1]]) %>%
      ggplot(aes(x = umap[,1], y = umap[,2])) +
      ggrastr::rasterise(geom_point(aes(color = expr), size = 0.05, stroke = 0), dpi = 300) +
      geom_density2d(data = filter(rbind(de_regions_maps, de_regions_maps2), gene == key[[1]]), breaks = 0.1, linewidth = 0.5,
                     contour_var = "ndensity", color = "black",
                     h = apply(umap_fit, 2, MASS::bandwidth.nrd) * 0.9,
                     show.legend = FALSE) +
...
plot_assemble(add_text("Expression pattern of selected genes under panobinostat treatment", x = 1.5, y = 1.5, vjust = 1, fontsize = font_size, fontface = "bold"),
              ...
              width = 170, height = 80, units = "mm", show_grid_lines = FALSE, latex_support = TRUE,
...
plot_assemble(
  ...
  add_graphic("../illustrations/manual_divergent_colorscaleArtboard 1.pdf", x = 85,  y = 105, units = "mm"),
  ...
  # add_text("(F) Panobinostat stimulates PDIA4 up-regulation in a subpopulation of tumor cells", x = 1.5, y = 150, vjust = 1, fontsize = font_size, fontface = "bold"),
  ...
  # add_plot(cowplot::get_legend(volcano_enrichment_pl + labs(color = "")), x = 134, y = 156.5, width = 36, height = 4),
...
de_plots2 <- de_plot_data2 %>%
  ...
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    ggrastr::rasterise(geom_point(aes(color = de), size = 0.05, stroke = 0, show.legend = FALSE), dpi = 600) +
    geom_density2d(data = de_regions_maps2 %>% mutate(inside = "in"),  breaks = 0.2, linewidth = 0.75,
                   contour_var = "ndensity", color = "black",
                   h = apply(umap_fit, 2, MASS::bandwidth.nrd) * 0.9,
                   show.legend = FALSE) +
    scale_color_gradient2(limits = c(-1, 1), oob = scales::squish, breaks = c(-1, 0, 1), na.value = "#00000000") +
    facet_wrap(vars(inside), nrow = 1, labeller = as_labeller(c("in" = "Inside Neighborhood", "out" = "Outside Neighborhood")))  +
    scale_x_continuous(limits = range(umap_fit[,1]) * 1.1) +
    scale_y_continuous(limits = range(umap_fit[,2]) * 1.1) +
    small_axis(label = "", fontsize = font_size_tiny, xlim = range(umap_fit[,1]), ylim = range(umap_fit[,2]), arrow_length = 3) +
...
```{r, paged.print = FALSE}
genes_of_interest <- c("ENSG00000155660")
...
comparison_pl2 <-  comparison_data2 %>%
  ggplot(aes(x = condition_short, y = expr)) +
    ...
    geom_segment(data = diff_df, aes(x = 1.5, y = ctrl, xend = 1.5, yend = panobinostat), color = "red", linewidth = 0.8) +
...
diff_pl <- diff_df %>% 
  ggplot(aes(x = origin, xend = origin, y = 0, yend = panobinostat - ctrl)) +
    ...
    geom_segment(color = "red", linewidth = 0.8) +
...
```{r, paged.print=FALSE}
glm_fit <- glmGamPoi::glm_gp(psce_pdia4[,psce_pdia4$condition == "ctrl"], design = ~ patient_id + inside, size_factors = "ratio", verbose = TRUE)
glm_de_res <- glmGamPoi::test_de(glm_fit, cond(inside = TRUE) - cond(inside = FALSE))
glm_fit2 <- lemur:::edger_fit(counts(psce_pdia4[,psce_pdia4$condition == "ctrl"]), design = ~ patient_id + inside, col_data = colData(psce_pdia4[,psce_pdia4$condition == "ctrl"]),
                             offset = log(glmGamPoi:::estimate_size_factors(counts(psce_pdia4[,psce_pdia4$condition == "ctrl"]), method = "ratio")))
...
volcano_enrichment_pl <- as_tibble(glm_de_res) %>%
  ...
  mutate(cyto_trans_related = name %in% ego1@geneSets[["GO:0002181"]]) %>%
  mutate(category = case_when(
    er_stress_related ~ "ER Stress",
    cyto_trans_related ~ "Cytopl. Transl.",
    TRUE ~ "other"
  )) %>%
  ...
  ggplot(aes(x = lfc, y = pval)) +
    ...
    scale_color_manual(values = c("ER Stress" = "#023FA5", "Cytopl. Transl." = "#8E063B")) +
    scale_y_continuous(limits = c(1, NA), expand = expansion(mult = c(0, 0.05)), trans = neg_log10_trans,
