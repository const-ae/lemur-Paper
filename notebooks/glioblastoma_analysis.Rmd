...
mc_biplot_marker <- as_tibble(colData(fit_small)) %>%
  ...
  ggplot(aes(x = emb[,1], y = emb[,2])) +
    ...
    geom_text(data = manual_text_pos %>% filter(gene != "HIST3H2A"), aes(x = V1, y = V2, hjust = hjust, vjust = vjust, label = gene_label), size = font_size_small / .pt) +
...
de_plots <- de_plot_data %>%
  ...
  group_map(\(data, key){
    ...
    ggplot(data, aes(x = umap[,1], y = umap[,2])) +
      ...
      facet_grid(vars(inside), vars(gene), labeller = labeller(inside =as_labeller( c("in" = "Inside Neighb.", "out" = "Outside Neighb")),
...
plot_assemble(
  ...
  add_text("(E) Predicted differential expression of \\emph{HIST3H2A} for each cell", x = 86.5, y = 111, vjust = 1, fontsize = font_size, fontface = "bold"),
  ...
  add_text("(F) Panobinostat stimulates \\emph{LMO2} down-regulation in a subpopulation of tumor cells", x = 1.5, y = 158, vjust = 1, fontsize = font_size, fontface = "bold"),
