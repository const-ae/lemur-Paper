...
umap_pl <- as_tibble(colData(fit_subset)) %>%
  mutate(umap = umaps$umap_all) %>%
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    ggrastr::rasterize(geom_point(size = 0.1, stroke = 0, color = "grey85"), dpi = 600) +
    geom_point(data = tibble(umap = umaps$umap_inter1[sel_pos,], position = seq(0, 1, l = n_pos)), aes(fill = position), shape = 21, color = "black", size = 0.8, stroke = 0.1) +
    colorspace::scale_fill_continuous_divergingx(palette = "Zissou1", mid = 0.5) +
    ggnewscale::new_scale_fill() +
    geom_point(data = tibble(umap = umaps$umap_inter2[sel_pos,], position = seq(0, 1, l = n_pos)), aes(fill = position), shape = 21, color = "black", size = 0.8, stroke = 0.1) +
...
pred_plot <- pred_expr_df %>%
  ggplot(aes(x = timepoint, y = pred)) +
    ...
    geom_segment(data = tibble(x = 36, y = 1.5, xend = 35, yend = 0.758, gene_short_name = "iqsec1b"),
                 aes(x = x, y = y, xend = xend, yend = yend)) +
    facet_wrap(vars(gene_short_name), scales = "fixed", labeller = as_labeller(\(x)glue("\\emph{{{x}}}"))) +
