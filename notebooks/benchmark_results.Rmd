...
suppl_int_signal_plt <- perf_data %>%
  ...
  mutate(metric = factor(metric, levels = c("ARI",  "AMI", "NMI", "celltypeSS_over_totalSS"),
                         labels = c("ARI $\\uparrow$", "AMI $\\uparrow$", "NMI $\\uparrow$", "Var. explained by cell type $\\uparrow$"))) %>%
  ggplot(aes(x = method, y = value)) +
    ggbeeswarm::geom_quasirandom(size = 0.4, width = 0.3) +
    stat_summary(geom = "pointrange", fun.data = mean_se, color = "red", size = 1, fatten = 1) +
    ggh4x::facet_grid2(vars(comparison), vars(metric), scales = "free_y", independent = "y",
...
plot_assemble(
  ...
  add_plot(suppl_int_signal_plt + guides(color = "none"), x = 0, y = 60, width = 170, height = 60),
...
pl1 <- full_data_rel %>%
  ...
  ggplot(aes(x = method, y = rel_perf)) +
    ...
    theme(axis.title.x = element_blank(), plot.title.position = "plot", 
          panel.spacing.y = unit(5, "mm")) +
    labs(title = "(A) Distances between predicted and observed expression", y = "distance (normalized per dataset)")
...
pl2 <- full_data_rel %>%
  ...
  ggplot(aes(x = method, y = value)) +
    ...
    labs(title = "(B) Correlation and maximum mean discrepancy between predicted and observed expression", y = "correlation")
...
prec_recall_plot <- prec_recall_df %>%
  ...
  ggplot(aes(x = recall, y = precision)) +
    ...
    coord_cartesian(clip = "off") +
    labs(title = "(H) Overlap of neighborhoods with ground truth",
...
plot_assemble(
  ...
  add_plot(perf_pred_plot, x = 110, y = 50, width = 60, height = 40),
