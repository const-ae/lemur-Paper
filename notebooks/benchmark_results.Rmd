...
method_labels <-  c("PCA" = "PCA", "harmony" = "Harmony", "invertible_harmony" = "Param. Harmony",
                    "multiCondPCA" = "multi-cond. PCA", "lemur" = "LEMUR", "CPA" = "CPA", "scvi" = "scVI")
```
...
suppl_int_mix_plt <- perf_data %>%
  ...
  ggplot(aes(x = method, y = value)) +
    ...
    stat_summary(geom = "point", fun = mean, color = "red", size = 1.4) +
...
int_bio_pl <- int_bio_df %>%
  ...
  unnest(value) %>%
  ggplot(aes(x = value[,1], y = value[,2])) +
    geom_density2d(aes(color = stage(method, after_scale = colorspace::lighten(colour, 0.3))), adjust = 3, contour_var = "ndensity", breaks = seq(0.9, 0.2, length.out = 3)) +
    geom_point(data = int_bio_df, aes(x =  mix / avg_mix, y = ARI / avg_ARI, color = method), alpha = 0.4, size = 0.8, stroke = 0) +
...
kang_plot <- kang_vis %>%
  ...
  ggplot(aes(x = umap1, y = umap2)) +
    ...
    scale_color_manual(values = c("ctrl" = "#FC8D62", "stim" = "#8DA0CB"), labels = c("ctrl" = "control", "stim" = "stimulated")) +
...
pred_method_labels <-  c("linear" = "Linear", "PCA" = "PCA", "harmony" = "Harmony", "invertible_harmony" = "Param. Harmony",
                    "multiCondPCA" = "multi-cond PCA", "lemur" = "LEMUR", "CPA" = "CPA", "scvi" = "scVI", "no_change" = "Identity")
...
pl1 <- full_data_rel %>%
  ...
  ggplot(aes(x = method, y = rel_perf)) +
    ...
    geom_point(data = full_data_rel %>% filter(metric %in% norm_metric) %>% 
                      summarize(rel_perf = mean(rel_perf), .by = c(method, metric)),
                    aes(y = rel_perf), color = "red", size = 1.4) +
    ...
    labs(title = "Distance and correlation measures between predicted and observed expression",
         ...
         y = "distance (standardized per dataset)")
...
pl2 <- full_data_rel %>%
  ...
  ggplot(aes(x = method, y = value)) +
    ...
    geom_point(aes(alpha = is.infinite(rel_perf) * 1.0), size = 0.2, show.legend = FALSE) +
    ggbeeswarm::geom_quasirandom(size = 0.2) +
    geom_point(data = full_data_rel %>% filter(str_starts(metric, "r2")) %>%
                      summarize(value = mean(value), .by = c(method, metric)),
                    aes(y = value), color = "red", size = 1.4) +
...
pl3 <- full_data_rel %>%
  ...
  ggplot(aes(x = method, y = value)) +
    ggbeeswarm::geom_quasirandom(size = 0.2) +
    geom_point(data = full_data_rel %>% filter(metric == "mmd") %>% summarize(value = mean(value), .by = c(method, metric)),
                    aes(y = value), color = "red", size = 1.4) +
    facet_grid(vars(is_perf), vars(metric), scales = "free_y", labeller = labeller(metric = as_labeller(pred_metrics_labels))) +
    scale_x_discrete(labels = pred_method_labels) +
    coord_cartesian(clip = "off") +
...
de_simulated_dat <- as_tibble(colData(lemur_fit)) %>%
  mutate(umap) %>%
...
de_simulated_pl <- de_simulated_dat %>%
...
fdr_control_plots <- de_power_fdr_with_label %>%
  ...
  group_map(\(data, key){
    ggplot(data, aes(x = nominal_fdr, y = FDR, group = paste0(data, vals, method_label))) +
      ...
      labs(x = "Nominal FDR", y = "average $\\frac{\\textrm{FP}}{\\textrm{FP}+\\textrm{TP}}$") +
...
all_methods_power <- de_power_fdr_with_label %>%
  ...
  ggplot(aes(x = rel_TPR, y = method_label)) +
    ...
    stat_summary(aes(color = controls_fdr), geom = "point", fun = mean, size = 2) +
...
fdr_power_dat <- all_de_power_fdr %>%
  ...
  mutate(data = fct_reorder(as.factor(data), lemur_fdr, max),
         ...
         vals = fct_recode(vals, control = "ctrl", stimulated = "stim", formalin = "form")
         ) 
...
var_duration_pl1 <- var_expl %>%
  ...
  mutate(method = factor(method, levels = c("lemur", "pca", "lemur_landmark", "lemur_harmony"), 
                         labels = c("LEMUR", "PCA", "LEMUR + Landmark Integration", "LEMUR + Harmony Integration"))) %>%
  mutate(is_row_subset = n_genes == 500) %>%
  ...
  group_map(\(data, key){
    ...
    ggplot(data, aes(x = dimensions, y = elapsed)) +
      ...
      scale_color_manual(values = scales::hue_pal()(4) |> 
               magrittr::set_names(c("LEMUR", "LEMUR + Landmark Integration", "PCA", "LEMUR + Harmony Integration"))) +
      scale_linetype_manual(values = c("LEMUR" = "solid", "LEMUR + Landmark Integration"  = "22", 
                                       "PCA"  = "22", "LEMUR + Harmony Integration"  = "solid")) +
...
```{r}
writexl::write_xlsx(list("Panel A" = kang_vis, 
                         "Panel B" = int_bio_df,
                         "Panel C" = kang_pred,
                         "Panel D" = pred_data_long,
                         "Panel E,F" = unpack_2d(de_simulated_dat),
                         "Panel G" = all_de_power_fdr),
                    path = "../source_data/performance_validation.xlsx")
```
