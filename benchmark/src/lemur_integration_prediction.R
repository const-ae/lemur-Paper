...
if(skip_multi_cond_pca){
  for(lvl in config$contrast){
    ...
    predict_results[[lvl]] <- predict(fit, alignment_design_matrix = alignment_design_matrix)
    predict_holdout_results[[lvl]] <- predict(holdout_fit, alignment_design_matrix = alignment_design_matrix2)
  }
}else{
  for(lvl in config$contrast){
    predict_results[[lvl]] <- predict(fit, newcondition = cond(!!config$main_covariate := lvl))
    predict_holdout_results[[lvl]] <- predict(holdout_fit, newcondition = cond(!!config$main_covariate := lvl))
  }
}
