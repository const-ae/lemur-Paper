...
pa <- argparser::arg_parser("Run differential expression per cluster")
...
pa <- argparser::add_argument(pa, "--n_cluster", type = "integer", default = 10, nargs = 1, help = "The seed")
...
group_var <- igraph::cut_at(clustering, pa$n_cluster)
...
de_res <- purrr::list_rbind(lapply(unique(group_var), \(lvl){
  ...
  psce_data <- psce_celltype[,psce_celltype$group == lvl]
  de <- tryCatch({
