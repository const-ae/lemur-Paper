...
pa <- argparser::arg_parser("Run clustering over all cells")
...
psce_data <- glmGamPoi::pseudobulk(sce, group_by = glmGamPoi::vars(fake_condition, sample),
                                   aggregation_functions = list(counts = "rowSums2", .default = "rowMeans2"))
res <- if(pa$test_method == "glmGamPoi"){
  fit <- glmGamPoi::glm_gp(psce_data, ~ fake_condition, size_factors = "ratio", verbose = TRUE)
  res <- glmGamPoi::test_de(fit, contrast = cond(fake_condition = "fake_ctrl") - cond(fake_condition = "fake_trt"))
  ...
  res <- data.frame(name = rownames(edger_res), pval = edger_res$PValue, adj_pval = edger_res$FDR,
